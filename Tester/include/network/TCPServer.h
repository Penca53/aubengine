...
public:
		...
		_AsioAcceptor.async_accept(
			[this](std::error_code ec, asio::ip::tcp::socket socket)
			{
				if (!ec)
				{
					...
					auto newConnection = std::make_shared<ClientSession<T>>(std::move(socket), _AsioContext, _messagesIn);
	...
	void SendMessageTo(const Message<T>& msg, std::shared_ptr<ClientSession<T>> client)
	{
	...
	void SendMessageToAll(const Message<T>& msg, std::shared_ptr<ClientSession<T>> ignoreClient = nullptr)
	{
